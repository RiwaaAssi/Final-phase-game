...
function checkMatch() {
    ...
    gameHistory.push({
        playerName: currentPlayer === 'player1' ? 'Player1' : 'Player2',
        number: firstCard.number,
        success
    });

    if (document.querySelectorAll('.matched').length === 18) {
        const winner = score_player1 > score_player2 ? 'Player 1' : 'Player 2';
        document.querySelector(".winner h2").textContent = `The Winner is ${winner}`;
        document.querySelector(".square-cards h3").textContent = score_player1 + score_player2;
...
function checkMatch() {
    const [firstCard, secondCard] = openGrids;  // array destruct -> const firstCard=openGrids[0];
    let success = false;
    ...
    localStorage.setItem('gameHistory', JSON.stringify(gameHistory));// convert array->json
